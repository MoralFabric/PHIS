    ...
    (async()=>{
      try{
        ...
        if(r?.value){
          ...
          const newOnes=allBase.filter(s=>!savedIds.has(s.id));
          const merged=newOnes.length>0?[...saved,...newOnes]:saved;
          if(newOnes.length>0)await window.storage.set(SK,JSON.stringify(merged));
          setStories(merged.map(normalizeStory));
        }else{
          const allBase=[...SEEDS,...EXTENDED_SOAR];
          setStories(allBase.map(normalizeStory));
          await window.storage.set(SK,JSON.stringify(allBase));
        }
      }catch(e){setStories([...SEEDS,...EXTENDED_SOAR].map(normalizeStory));}
      // load experience
      try{
        const er=await window.storage.get(EXP_SK);
        if(er?.value) setExperience(JSON.parse(er.value));
      }catch(e){}
      setLoading(false);
    })();
  },[]);

  async function persist(d){try{await window.storage.set(SK,JSON.stringify(d));}catch(e){}}
  async function persistExp(d){try{await window.storage.set(EXP_SK,JSON.stringify(d));}catch(e){}}
