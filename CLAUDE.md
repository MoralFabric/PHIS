...
**Narration quality is the visitor's machine, not ours.** Chrome on Windows exposes only five legacy SAPI voices; Edge exposes roughly 100 cloud neural ones; macOS has its own set. `scoreVoice()` ranks whatever is present rather than matching exact names. **Accent is weighted above quality on purpose**: `en-CA` +60, `en-US` +45, every other English locale -50. Adam is Canadian, Canadian English sits with General American rather than with Received Pronunciation, and a polished British voice reading his words in the first person is worse than a plainer North American one. An earlier version gave `en-GB` +6 and `en-US` nothing, so "Google UK English Male" won in Chrome; the neural bonus of +100 also swamped a locale bonus of +12, so any British "Online (Natural)" voice won on Edge; and the preferred-names list contained Daniel, Oliver, Arthur and Matilda, which are British and Australian. All three paths led to a British narrator. The picker labels now carry a locale tag ("Liam (CA)", "Guy (US)") so the accent is visible rather than hidden behind the name. Exact matching never hit on Windows or Edge, because real voice names look like `Microsoft Guy Online (Natural) - English (United States)`, so every lookup fell through to `pool[0]` and landed on Microsoft David.
