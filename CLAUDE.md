...
**Narration quality is the visitor's machine, not ours.** Chrome on Windows exposes only five legacy SAPI voices; Edge exposes roughly 100 cloud neural ones; macOS has its own set. **The film script is third person.** The narrator is a synthetic voice that will not sound like Adam on anyone machine, so it does not claim to be him: "Adam Waldman turns data into decisions", not "I am Adam Waldman". The typed lines inside the paste, fill, write and ask scenes stay in first person on purpose, because those depict the product input and output rather than the narrator. If the script ever moves back to first person, restore the accent-first weighting in scoreVoice, which is in git history at 1adbe68.

`scoreVoice()` ranks whatever is present rather than matching exact names. Quality leads (+100 for neural), with accent as a mild tiebreak: en-CA +14, en-US +10. It briefly ran accent-first when the script was in first person, which is the right weighting only in that case.
...
*Version 4 was rhythmically right but musically dull.* Bare triads over D G A D read as stock corporate backing. The current tune keeps the rhythmic engine and changes the music: a vi-IV-I-V shape with add9 and sus colour instead of plain triads, a syncopated four note hook on steps 0, 3, 6 and 10 rather than notes on the beat, and a dotted eighth delay. 171bpm.

**Narration must wait for the voice list.** Chrome fills `getVoices()` asynchronously. The film autoplays immediately, so scene one used to speak before the list settled, falling back to `listVoices()[0]` from a partial list, and every later scene used a different voice. The narration effect is gated on `voiceURI`, which is one of its dependencies, so the line speaks as soon as the list arrives. Verify by wrapping `speechSynthesis.speak` and asserting one distinct voice across all utterances.

**You cannot hear it, so verify it structurally.**
