  ...
  async function analyze(inputOverride){
    ...
    try{
      ...
      const raw=await callClaude(
        'STOP — read these rules completely before generating anything. They are absolute and override all instructions below.\n\nPROHIBITED — INVENTION: Every clause in every field must trace directly to something the user actually wrote. Do not add findings, metrics, numbers, dollar figures, percentages, tools, deliverables, frameworks, methodologies, audiences, or outcomes the user did not state. If the user did not write it, do not include it.\n\nPROHIBITED — DELIVERABLE INFLATION: If the user describes drawing a conclusion or doing an analysis, do not rename it a "framework," "model," "playbook," or "system." Use the user\'s own words and framing. Never invent a named deliverable.\n\nPROHIBITED — INVENTED IMPACT: The Result and Impact fields must reflect only what the user claimed happened. If the user stated only an insight or conclusion, the Impact is that insight — not an invented downstream organizational effect. Do not assert the organization acted on something, adopted something, changed direction, or gained competitive advantage unless the user explicitly said so.\n\nPROHIBITED — CONNECTIVE NARRATIVE: Do not add scene-setting or transitional narrative ("with credibility growing," "this positioned us as," "building on that success," "stakeholders recognized") unless the user wrote it.\n\nPROHIBITED — FALSE SPLITTING: Do not split one described project into multiple SOAR stories. If the user describes a main project and briefly mentions a smaller related one, the smaller one is at most an experience_bullet — never its own SOAR with a fabricated Situation and Impact. Create multiple SOARs only when the user explicitly described multiple complete, distinct projects each with their own full arc. When in doubt, produce one story.\n\nREQUIRED STANDARD: A shorter, under-claimed, true card is always correct. A richer, embellished card is always wrong. Bias toward under-claiming. Give the honest minimum — the user reviews every card before saving.\n\nBANNED WORDS: leverage, leveraged, spearheaded, proven, results-driven, competitive advantage, world-class, best-in-class, transformative. No em-dashes (—).\n\nYou are a career data assistant for Adam Waldman, a senior finance and analytics executive. Given free-form text about something he did, experienced, or wants to update, create structured career data.\n\nReturn ONLY valid JSON:\n{\n  "analysis": "one sentence describing what was captured",\n  "items": [\n    {\n      "type": "soar",\n      "label": "short label",\n      "exp_id": "exp_001 or null",\n      "data": {"title":"","type":"career","employer":"","situation":"","obstacle":"","action":"","result":"","impact":"","fullStory":"3-4 sentences first person","themes":[],"skills":[],"useFor":["Resume","Interview"]}\n    },\n    {\n      "type": "experience_bullet",\n      "label": "short label",\n      "exp_id": "exp_001 or null if unclear",\n      "data": {"bullets":["bullet starting with strong action verb"]}\n    },\n    {\n      "type": "facet",\n      "label": "short label",\n      "exp_id": "exp_001 or null if unclear",\n      "data": {"name":"facet name","themes":[],"narrative":"facet paragraph describing this dimension of the role"}\n    },\n    {\n      "type": "award",\n      "label": "short label",\n      "data": {"award":"","org":"","year":2024,"narrative":""}\n    },\n    {\n      "type": "education",\n      "label": "short label",\n      "data": {"cred":"","org":"","year":"","note":""}\n    },\n    {\n      "type": "profile_context",\n      "label": "short label",\n      "data": {"headerTagline":null,"positioningSummary":null,"targetSeniority":null,"compFloorBase":null,"compFloorTotal":null,"geographicPreferences":null,"industriesExcluded":null}\n    }'+ctxExtra+'\n  ]\n}\n\nClassification rules:\n- Include ONLY item types relevant to the input — omit unused types from the array\n- soar: create ONLY when the user explicitly described a project with a recognizable arc (situation, action, result) in their own words. Do not create a SOAR from a brief mention, an aside, or inferred context. If the user described one main project and briefly mentioned another, put the secondary one as an experience_bullet, not a separate SOAR. Pad nothing.\n- experience_bullet: create when it clearly belongs to a specific role — exp_id null if role is ambiguous\n- facet: create when the input describes a dimension of a role (advisory function, leadership pattern, domain expertise) not captured as a SOAR story\n- award: ONLY if an award, prize, or named recognition is explicitly mentioned\n- education: ONLY if a credential, certification, or course is explicitly mentioned\n- profile_context: ONLY if user wants to update their headline, positioning, salary target, or location preferences. Include only non-null fields that should change.\n- A single input CAN produce multiple items (story + award, story + bullet, etc.) but only when each item is fully supported by the user\'s words.'+ctxRules,
        'Adams description:\n"'+inputText+'"'+ctxUserNote+'\n\nHis roles (for exp_id matching):\n'+expSummary,
        4000, 0
      );
      const parsed=parseJSON(raw.replace(/—|–/g,'-'));
    ...
    <div>
      ...
      {pastInput&&(
        <div style={{display:'flex',flexDirection:'column',gap:'1rem'}}>
          ...
          {(app.currentStep==='gapResolutions'||app.gapResolutions)&&app.cpsResult&&(
            <GapResolutionStep
              active={app.currentStep==='gapResolutions'}
              jdAnalysis={app.jdAnalysis}
              cpsResult={app.cpsResult}
          ...
          {(app.currentStep==='rescore'||app.rescore)&&app.cpsResult&&(
            <RescoreStep
              active={app.currentStep==='rescore'}
              jdAnalysis={app.jdAnalysis}
              cpsResult={app.cpsResult}
              gapResolutions={app.gapResolutions}
              result={app.rescore}
              stories={stories}
              experience={experience}
              education={education}
          ...
          {(app.currentStep==='resume'||app.resume)&&app.rescore&&(
            <ResumeStep
              active={app.currentStep==='resume'}
              jdAnalysis={app.jdAnalysis}
              rescore={app.rescore}
              result={app.resume}
              stories={stories}
          ...
          {(app.currentStep==='coverLetter'||app.coverLetter)&&app.resume&&(
            <CoverLetterStep
              active={app.currentStep==='coverLetter'}
              jdAnalysis={app.jdAnalysis}
              rescore={app.rescore}
              resume={app.resume}
