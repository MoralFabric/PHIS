...
export async function insertAward(a) {
  const { error } = await supabase.from('awards').insert({
    award:        a.award        || '',
    year:         String(a.year  || ''),
    organization: a.org          || '',
    narrative:    a.narrative    || '',
    jd_themes:    a.jdThemes     || [],
  })
  if (error) throw error
}
...
export async function insertEducation(e) {
...
export async function createGuestSession(info) {
  const { data, error } = await supabase
    .from('guest_sessions')
    .insert({ name: info.name, email: info.email || null, company: info.company || null, role: info.role || null })
    .select()
    .single();
  if (error) { console.error('createGuestSession', error); return null; }
  return data;
}

export async function logFitRun(sessionId, input, roleUnderstood) {
  if (!sessionId) return;
  const { data: cur } = await supabase.from('guest_sessions').select('fit_runs').eq('id', sessionId).single();
  const runs = (cur && cur.fit_runs) || [];
  runs.push({ input, role_understood: roleUnderstood, at: new Date().toISOString() });
  const { error } = await supabase.from('guest_sessions').update({ fit_runs: runs, updated_at: new Date().toISOString() }).eq('id', sessionId);
  if (error) console.error('logFitRun', error);
}

export async function logGuestQuestion(sessionId, question) {
  if (!sessionId) return;
  const { data: cur } = await supabase.from('guest_sessions').select('questions').eq('id', sessionId).single();
  const qs = (cur && cur.questions) || [];
  qs.push({ question, at: new Date().toISOString() });
  const { error } = await supabase.from('guest_sessions').update({ questions: qs, updated_at: new Date().toISOString() }).eq('id', sessionId);
  if (error) console.error('logGuestQuestion', error);
}
...
export async function getGuestSessions() {
  ...
  if (error) { console.error('getGuestSessions', error); return []; }
  return data || [];
}

export async function logInterviewQuestion(sessionId, question, answer) {
  if (!sessionId) return;
  const { data: cur } = await supabase.from('guest_sessions').select('interview_questions').eq('id', sessionId).single();
  const qs = (cur && cur.interview_questions) || [];
  qs.push({ question, answer: answer || null, at: new Date().toISOString() });
  const { error } = await supabase.from('guest_sessions').update({ interview_questions: qs, updated_at: new Date().toISOString() }).eq('id', sessionId);
  if (error) console.error('logInterviewQuestion', error);
}
...
export async function getMetrics() {
  ...
  if (error) {
    console.error('getMetrics error (table may not exist):', error);
    return [];
  }
  console.log('getMetrics result:', data);
  return data || [];
}

export async function createMetric(obj) {
  const { data, error } = await supabase
    .from('profile_metrics')
    .insert({ value: obj.value, label: obj.label, pinned: obj.pinned || false, tags: obj.tags || [], sort_order: obj.sort_order || 0 })
    .select()
    .single();
...
export async function updateMetric(id, obj) {
  const { error } = await supabase.from('profile_metrics').update(obj).eq('id', id);
  if (error) console.error('updateMetric', error);
}

export async function deleteMetric(id) {
  const { error } = await supabase.from('profile_metrics').delete().eq('id', id);
  if (error) console.error('deleteMetric', error);
}

// ── Profile values ──

export async function getValues({ surface, kind } = {}) {
  let q = supabase.from('profile_values').select('*').order('sort_order', { ascending: true });
  if (surface) q = q.eq('surface', surface);
  if (kind)    q = q.eq('kind', kind);
  const { data, error } = await q;
  if (error) { console.error('getValues error:', error); return []; }
  return data || [];
}

export async function insertValue(row) {
  const { data, error } = await supabase.from('profile_values').insert(row).select().single();
  if (error) { console.error('insertValue', error); return null; }
  return data;
}

export async function updateValue(id, updates) {
  const { data, error } = await supabase.from('profile_values').update(updates).eq('id', id).select().single();
  if (error) { console.error('updateValue', error); return null; }
  return data;
}

export async function deleteValue(id) {
  const { error } = await supabase.from('profile_values').delete().eq('id', id);
  if (error) console.error('deleteValue', error);
}
...
export async function getGuidance() {
  const { data, error } = await supabase
    .from('interview_guidance')
    .select('*')
    .eq('active', true)
    .order('created_at', { ascending: true });
  if (error) { console.error('getGuidance error:', error); return []; }
  return data || [];
}

export async function insertGuidance(row) {
  const { data, error } = await supabase.from('interview_guidance').insert(row).select().single();
  if (error) { console.error('insertGuidance', error); return null; }
  return data;
}
